...
test('completed and flagged batch state persists across reloads', async ({ page }) => {
  await mockHarmonizeSuccess(page);

  // Given: review rows are available
  const fileId = await uploadAndAnalyze(page, fileFixture('basic.csv'));
  const preOverrides = await page.request.get(`/stage-4/overrides/${fileId}`);
  expect(await preOverrides.json()).toBeNull();
  await clickHarmonize(page);
  ...
  // When: the current batch is marked complete, then flagged
  await page.click('#completeBatchButton');
  await page.waitForResponse((response) => response.url().includes('/stage-4/overrides') && response.ok());
  await expect(page.locator('#completeBatchButton')).toContainText(/Flag for review/i);
  await page.click('#completeBatchButton');
  await page.waitForResponse((response) => response.url().includes('/stage-4/overrides') && response.ok());
  ...
  // Then: the flagged state persists
  await expect(page.locator('.batch-progress-item.flagged').first()).toBeVisible();
});
