...
def test_environment_accepts_the_parallel_bdf_foundation(tmp_path: Path) -> None:
    ...
    boundary = _run(
        "get", environment, "bdf", "staging", "deployer_boundary_arn"
    )
    ...
    assert boundary.stdout.strip() == (
        "arn:aws:iam::084828580051:policy/bdf-datachord-deployer-boundary"
    )
...
def test_environment_rejects_unknown_fields_and_mismatched_foundation_names(
        ...
        json.dumps(
            {
                **_document(),
                "application_role_boundary_arn": (
                    ...
                    "another-application-role-boundary"
                ),
            }
        ),
        encoding="utf-8",
    ...
    mismatched_boundary = _run(
        "validate", wrong_boundary, "netrias", "staging"
    )
...
def _bdf_document() -> dict[str, object]:
    return {
        ...
        "deployer_role_arn": (
            "arn:aws:iam::084828580051:role/foundation/bdf-datachord-deployer"
        ),
        "application_role_boundary_arn": (
            "arn:aws:iam::084828580051:policy/"
            "bdf-datachord-application-role-boundary"
        ),
        "application_role_path": "/application/",
