...
test_supported_stages() {
  local target_name stage_name

  for target_name in bdf netrias; do
    for stage_name in dev qa staging prod; do
      assert_equal "$stage_name" "$(require_stage_name "$target_name" "$stage_name")" "$target_name stage $stage_name"
    done
  done
  assert_fails_with "Choose a stage" require_stage_name bdf test
}
...
test_target_contract_values() {
  assert_equal "084828580051" "$(target_value bdf expected_account_id)" "BDF account"
  assert_equal "netrias-data-chord-tofu-state-945365518758-us-east-2" \
    "$(target_value netrias state_bucket_name)" "Netrias state bucket"
  assert_equal "/application/" "$(target_value bdf application_role_path)" "application IAM path"
}
...
test_backend_uses_contract_and_native_lock_file() {
  ...
  DATA_CHORD_TF_DATA_DIR="$(mktemp -d)" init_tofu netrias qa
  ...
  assert_equal "1" "$(grep -Fxc -- '-backend-config=key=datachord/netrias/qa/tofu.tfstate' "$tofu_calls")" "backend state key"
